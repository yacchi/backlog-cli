...
const resultCopy: Record<ResultType, {title: string; body: string; accent: string}> = {
  success: {
    title: '認証が完了しました',
...
function canAutoClose(): boolean {
  return typeof (window as {forceCloseTab?: () => void}).forceCloseTab === 'function'
}
...
export default function ResultView({type, message}: ResultViewProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setCountdown((prev) => {
        if (prev <= 1) {
          const closer = (window as {forceCloseTab?: () => void}).forceCloseTab
          closer?.()
