...
→ {
    ...
    "authorization_endpoint": "https://mcp.example.com/mcp/authorize",
    "token_endpoint": "https://mcp.example.com/mcp/token",
    "registration_endpoint": "https://mcp.example.com/mcp/register",
...
MCP クライアントは最初に DCR でクライアント登録を行う。
サーバーは `redirect_uris` + `client_name` + `iat` を JWE で暗号化して `client_id` として返す。
DB 不要のステートレス DCR。

```
POST /mcp/register
{ "redirect_uris": ["http://localhost:12345/callback"], "client_name": "Claude Desktop" }

→ 201
{ "client_id": "<JWE>", "redirect_uris": [...], "grant_types": ["authorization_code", "refresh_token"] }
```
...
```
┌──────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  Claude   │    │  MCP Server  │    │ Relay Server │    │   Backlog    │
└────┬─────┘    └──────┬───────┘    └──────┬───────┘    └──────┬───────┘
     │                 │                   │                   │
     ...
     │                 │                   │                   │
     │ 2. GET /mcp/authorize              │                   │
     │    ?client_id=JWE&redirect_uri=... │                   │
     │    &code_challenge=...&state=...   │                   │
     │────────────────>│                   │                   │
     │                 │                   │                   │
     │    302 → Backlog OAuth             │                   │
     │    state=JWE(AuthorizeState)       │                   │
     │<────────────────│                   │                   │
     │                 │                   │                   │
     │ 3. ユーザーが Backlog でログイン・認可                  │
     │────────────────────────────────────────────────────────>│
     │                 │                   │                   │
     │ 4. GET /mcp/authorize/callback?code=xxx&state=JWE      │
     │<────────────────────────────────────────────────────────│
     │────────────────>│                   │                   │
     │                 │                   │                   │
     │                 │ 5. state JWE 復号 │                   │
     ...
     │                 │<──────────────────│                   │
     │                 │                   │                   │
     │                 │ 7. Backlog トークンを JWE ラップ       │
     │                 │    → MCP code (JWE)                   │
     │                 │                   │                   │
     │    302 → redirect_uri?code=JWE&state=...               │
     │<────────────────│                   │                   │
     │                 │                   │                   │
     │ 8. POST /mcp/token                 │                   │
     ...
     │────────────────>│                   │                   │
     │                 │                   │                   │
     │   { access_token: JWE, refresh_token: JWE }            │
     │<────────────────│                   │                   │
```

- `access_token` = JWE(bl_access_token, space, domain, exp)
- `refresh_token` = JWE(bl_refresh_token, space, domain) — exp なし（refresh は relay 経由で更新）
- PKCE (S256) を使用。state は Claude 側が管理。

#### Scope によるテナント解決

`scope=backlog:mycompany.backlog.jp` でスペースとドメインを指定。
テナントが 1 つだけの場合は自動解決。
...
- [x] NodejsFunction + `commandHooks` での Lambda デプロイ構成 (CDK synth 確認済み)
- [x] テスト: sandbox セキュリティ E2E（Deno 実行環境が必要、12 件）

### Phase 4: 運用 + 拡張 ✅

- [x] サーバーエントリーポイント (`serve.ts` + `@hono/node-server`)
- [x] Dockerfile (マルチステージ + Lambda Web Adapter + Deno + Pyodide キャッシュ)
- [x] CDK スタック (`mcp-aws/`)
- [x] 組織向けセットアップ・設計ドキュメント (`docs/design/remote-mcp-server.md`)
