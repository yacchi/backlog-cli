...
| `gh pr merge <number>`    | `backlog pr merge <number>`   | PRマージ             |
...
| `backlog wiki delete <id>` | (API経由のみ)     | Wiki削除   |
...
| `backlog markdown logs`    | マークダウン変換ログ表示          |
| `backlog ai prompt`        | AI要約プロンプト最適化・管理       |
...
| -                 | `--mine`           | ✅ Backlog独自            |
| -                 | `--count`          | ✅ Backlog独自            |
| -                 | `--type / -T`      | ✅ Backlog独自 (種別フィルタ)   |
| -                 | `--sort`           | ✅ Backlog独自 (ソートフィールド) |
| -                 | `--order`          | ✅ Backlog独自 (asc/desc) |
| -                 | `--summary`        | ✅ Backlog独自 (AI要約)     |
| -                 | `--markdown`       | ✅ Backlog独自 (記法変換)     |
| -                 | `-L 0`             | ✅ Backlog独自 (全件取得)     |
...
| `--resolution`         | 完了理由指定                         |
| `--sort`, `--order`    | ソートフィールド・順序指定                   |
| `-L 0`                 | 全件取得（自動ページネーション）               |
| `backlog ai prompt`    | AI要約プロンプトの最適化・適用                |
| `backlog api`          | 汎用APIリクエスト (gh api 相当)          |
...
| Wikiを削除    | `backlog wiki delete <id>`          |
...
| PRを編集      | `backlog pr edit <number> -R <repo> -t "New title"`      |
| PRをマージ     | `backlog pr merge <number> -R <repo>`                    |
...
# 汎用APIリクエスト（gh api 相当）
backlog api /api/v2/space
backlog api /api/v2/issues -F "projectId[]=12345"

# AI機能
backlog ai prompt optimize

# マークダウン移行
backlog markdown migrate init PROJECT
