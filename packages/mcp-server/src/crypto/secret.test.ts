...
describe("deriveEncKey", () => {
    it("derives a 32-byte key deterministically", () => {
        const k1 = deriveEncKey(D1);
        const k2 = deriveEncKey(D1);
        expect(k1.length).toBe(32);
        expect(Buffer.from(k1).equals(Buffer.from(k2))).toBe(true);
    });

    it("different d yields different key", () => {
        const k1 = deriveEncKey(D1);
        const k2 = deriveEncKey(D2);
...
describe("seal/open", () => {
    ...
    it("fails with DecryptError on wrong key", async () => {
        const jwe = await seal("secret", deriveEncKey(D1), "kid-1", "x.backlog.jp", "at");
        await expect(open(jwe, () => deriveEncKey(D2))).rejects.toBeInstanceOf(DecryptError);
    });
    ...
    it("resolves the key by kid from the header", async () => {
        const key1 = deriveEncKey(D1);
        const key2 = deriveEncKey(D2);
