 ...
 *
 * Serves the install.sh script with the relay URL injected,
 * so `curl -fsSL https://relay.example.com/install.sh | sh` works
 * without the user needing to specify --relay-url.
 */
...
# Auto-injected relay URL (set when served from relay server)
: "\${BACKLOG_RELAY_URL:=__RELAY_URL__}"
export BACKLOG_RELAY_URL
...
install_from_releases() {
    ...
    local os arch archive_name url tmpdir
    ...
    local checksum_url="https://github.com/\${GITHUB_REPO}/releases/download/\${latest}/checksums.txt"
...
/**
 * Create install script handler.
 */
export function createInstallHandlers(config: RelayConfig): Hono {
  const app = new Hono();

  app.get("/install.sh", (c) => {
    const reqCtx = extractRequestContext(c);
    const relayUrl = config.server.base_url || reqCtx.baseUrl;

    const script = INSTALL_SCRIPT.replace(/__RELAY_URL__/g, relayUrl);
